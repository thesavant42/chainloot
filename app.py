...
async def on_audio_chunk(chunk: cl.AudioChunk):
    ...
    if chunk.is_end:
        # Audio recording complete, process the audio
        logger.info(f"AUDIO DIAG: on_audio_chunk END - Session ID: {cl.context.session.id}")
        audio_buffer = cl.user_session.get("audio_buffer")
        if not audio_buffer:
...
async def on_audio_end():
    logger.info(f"AUDIO DIAG: on_audio_end triggered - Session ID: {cl.context.session.id}. Audio will be processed in on_audio_chunk.")
    return True
